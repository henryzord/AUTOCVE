...
from pbil.individuals import Skeleton
...
def execute_exp(
    ...
    try:
        ...
        try:
            ...
            jtrain_probs = env.make_object_array(train_probs.shape[0], env.find_class('[D'))
            for i in range(train_probs.shape[0]):
                row = env.make_double_array(train_probs[i, :])
            ...
            for i in range(test_probs.shape[0]):
                row = env.make_double_array(test_probs[i, :])
            ...
            # test_evaluation = EDAEvaluation.from_jobject(train_evaluation_obj, data=train_data, seed=seed)
            # clf = Classifier(classname='NonJavaClassifier', )
...
def main():
    ...
    parser.add_argument(
        '--time-per-task', action='store', required=False, type=int, default=5400,
    ...
    for id_exp in datasets_names:
        print("Problem " + id_exp)
        for id_trial in range(N_TRIALS):
            print("Trial " + str(id_trial))
            for n_fold in range(1, 10):  # iterates over folds
                # TODO join results!
                execute_exp(
                    n_fold=n_fold,
                    id_trial=id_trial,
